...
@register.filter(name='positive_reviews')
def positive_reviews(phone_number):
    allusers= phonemodel.objects.filter(phone_number=phone_number)
    all_danger_reviews=reviewmodel.objects.filter(reviewnumber= allusers[0], review_status__icontains="neutral")
